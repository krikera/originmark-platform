...
2. **Web Infrastructure:** A Next.js web dashboard for end-user verification (documented separately).
...
OriginMark uses **SQLAlchemy** for ORM and **Alembic** for schema migrations. The default database is SQLite, but the system is strictly RDBMS-agnostic and production deployments should use PostgreSQL.

### Core Tables
- `users`: Identity management (UUIDs, usernames, bcrypt hashes).
...
- `user_feedback`: Admin/telemetry metrics.
...
*   `POST /sign`: 
    ...
    *   **Query Params:** `author`, `model_used`, `private_key` (optional, auto-generated if omitted).
...
*   `POST /webhooks`: Register a new URL for asynchronous event dispatch (e.g., `signature.created`).
*   *(Note: Webhooks currently rely on `aiohttp` for non-blocking HTTP dispatch).*
